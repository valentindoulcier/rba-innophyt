...
function generateXML($root)
{
	...
	foreach ( $branchesList as $branch ) { 
  		if($branch->getAttribute("id") != $rootID)
  			$branchesToRemove[] = $branch;  // On ne peut pas supprimer directement ici apparament
	} 
	foreach( $branchesToRemove as $branch ){ 
  		$branch->parentNode->removeChild($branch); // Il faut passer par le père pour supprimer son fils
	} 
